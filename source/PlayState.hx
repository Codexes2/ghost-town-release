...
class PlayState extends MusicBeatState
{
	...
	var yuyukoTweenManager:FlxTweenManager = new FlxTweenManager();
	...
	override public function create()
	{
		...
		if (curStage == 'stage3' && SONG.player2 == 'yuyuko')
			add(yuyukoTweenManager);
		...
		var debug:Array<Float> = [];
		switch (curStage)
		{
			case 'stage1' | 'stage2' | 'stage3': // Stages of 2hu
				// debug = CodexesDebug.turnToFloat();
				var bg:BGSprite = new BGSprite('${curStage}', -1000, -169, 0.9, 0.9);
	...
	function startSong():Void
	{
		...
		// some real hard checks on this one
		if (curStage == 'stage3' && SONG.player2 == 'yuyuko' && yuyukoTweenManager != null)
		{
			// cherry tree stage and yuyuko, right?
			yuyukoTweenManager.tween(dad, {y: dad.y + 100}, 4.0, {ease: FlxEase.quadInOut, type: FlxTweenType.PINGPONG});
		}
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if (iconP2.winningIndex != -1)
				iconP2.animation.curAnim.curFrame = iconP2.winningIndex;
			if (iconP1.losingIndex != -1)
				iconP1.animation.curAnim.curFrame = iconP1.losingIndex;
		}
		else if (healthBar.percent > 80)
		{
			if (iconP1.winningIndex != -1)
				iconP1.animation.curAnim.curFrame = iconP1.winningIndex;
			if (iconP2.losingIndex != -1)
				iconP2.animation.curAnim.curFrame = iconP2.losingIndex;
		}
		else
		{
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (daNote.noteType == 'Sword Note')
		{
			new FlxTimer().start(0.001, function(tmr:FlxTimer)
			{
				// probably isn't fair to die while this is going off
				// when the song is ending, so i coded a hard limit
				if (health <= 0.01)
					health = 0.01;
				else
					health -= 0.0005;
